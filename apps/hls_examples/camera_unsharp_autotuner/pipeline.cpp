...
class MyPipeline {
    ...
    Func unsharp_img(Func input) {
        ...
        Func kernel, kernel_f, blur_y, blur_x, sum_y, sum_x, sharpen;
        ...
        if (false) {
            ...
            sum_y(x, y) += cast<uint16_t>(gray(x, y+win.x)) * kernel(win.x);
            blur_y(x, y) = cast<uint8_t>(sum_y(x, y) >> 8);
            sum_x(x, y) += cast<uint16_t>(blur_y(x+win.x, y)) * kernel(win.x);
        ...
        if (schedule == 1) {
            ...
            blur_y.compute_at(processed, tx).vectorize(x, 8);
...
public:
  MyPipeline(int _schedule, int32_t matrix[3][4], float gamma, float contrast, Image<uint16_t> in)
      : schedule(_schedule), input(in), curve("curve")
    {
...
public:
    MyPipelineOpt(int _schedule, int32_t matrix[3][4], float gamma, float contrast, Image<uint16_t> in)
        : schedule(_schedule), input(in), curve("curve")
    {
        // The camera pipe is specialized on the 2592x1968 images that
        // come in, so we'll just use an image instead of a uniform image.
...
int main(int argc, char **argv) {
    ...
    Image<uint16_t> input = load_image(argv[4]);
    Image<uint8_t> output(640*4, 480*4, 3);
    ...
    MyPipeline p1(1, matrix, gamma, contrast, input);
    p1.run_cpu(output);
    save_image(output, "out.png");

    MyPipelineOpt p2(2, matrix, gamma, contrast, input);
    p2.run_hls(output);
