#ifndef HALIDE_EXTRACT_HW_KERNEL_H
#define HALIDE_EXTRACT_HW_KERNEL_H
...
struct HWKernel {
    ...
    std::map<std::string, Box> unroll_boxes;
    std::string unroll_level; // it is the level on top of all unroll levels (itself is not unrolled)

    HWKernel() : is_inlined(false), is_pingpong(false), is_output(false) {}
    HWKernel(Function f, const std::string &s)
        : func(f), name(s), is_inlined(false), is_pingpong(false), is_output(false) {}
};
 ...
 */
Stmt extract_hw_kernel(Stmt s, const std::map<std::string, Function> &env,
