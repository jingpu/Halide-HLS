...
void CodeGen_C::compile(const LoweredFunc &f) {
    ...
    if (is_header() && f.linkage == LoweredFunc::ExternalPlusMetadata) {
    ...
    } else if (!is_header() && f.linkage == LoweredFunc::ExternalPlusMetadata) {
        // Emit definition of the argv version
        stream << "\n"
        ...
        // TODO(jingpu) add C definitionn for metadata_getter, see implementation in CodeGen_LLVM
    }
