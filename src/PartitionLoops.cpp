...
class FindInnerProductions : public IRVisitor {
    using IRVisitor::visit;
...
bool contains_hw_functions (Stmt s) {
    bool res = false;
    FindInnerProductions finder;
    s.accept(&finder);
    for(size_t i = 0; i < finder.inner_productions.size(); i++) {
        if (starts_with(finder.inner_productions[i], "_hls_target.")) {
            res = true;
            break;
        }
    }
