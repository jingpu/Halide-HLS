...
CodeGen_CatapultHLS_Target::CodeGen_CatapultHLS_Target(const string &name, Target target)
    : target_name(name),
      hdrc(hdr_stream,
           target.with_feature(Target::CPlusPlusMangling),
           CodeGen_CatapultHLS_C::CPlusPlusHeader),
      srcc(src_stream,
           target.with_feature(Target::CPlusPlusMangling),
           CodeGen_CatapultHLS_C::CPlusPlusImplementation) { }


CodeGen_CatapultHLS_Target::~CodeGen_CatapultHLS_Target() {
    hdr_stream << "#endif\n";
...
void CodeGen_CatapultHLS_Target::init_module() {
    debug(1) << "CodeGen_CatapultHLS_Target::init_module\n";
    ...
    string module_name = "HALIDE_CODEGEN_CATAPULTHLS_TARGET_" + target_name + "_H";
...
void CodeGen_CatapultHLS_Target::add_kernel(Stmt s,
                                    const string &name,
                                    const vector<CatapultHLS_Argument> &args) {
    debug(1) << "CodeGen_CatapultHLS_Target::add_kernel " << name << "\n";

    hdrc.add_kernel(s, name, args);
    srcc.add_kernel(s, name, args);
}

void CodeGen_CatapultHLS_Target::dump() {
    std::cerr << src_stream.str() << std::endl;
}


string CodeGen_CatapultHLS_Target::CodeGen_CatapultHLS_C::print_stencil_pragma(const string &name) {
    ostringstream oss;
...
void CodeGen_CatapultHLS_Target::CodeGen_CatapultHLS_C::add_kernel(Stmt stmt,
                                                   const string &name,
                                                   const vector<CatapultHLS_Argument> &args) {
  ...
  if (!is_header()) {
    ...
    for (size_t i = 0; i < args.size(); i++) {
        ...
        if (args[i].is_stencil) {
            CodeGen_CatapultHLS_Base::Stencil_Type stype = args[i].stencil_type;
...
void CodeGen_CatapultHLS_Target::CodeGen_CatapultHLS_C::visit(const For *op) {
...
void CodeGen_CatapultHLS_Target::CodeGen_CatapultHLS_C::visit(const Allocate *op) {
